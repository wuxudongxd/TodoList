...
router.post("/column", async (req, res) => {
  try {
    const { name } = req.body;
    // 执行SQL语句
    const sql = `INSERT INTO \`column\` (id, name) VALUES ('${uuidv4()}', '${name}');`;
    const result = await query(`${sql}`);
    res.json({ res: "success" });
  } catch (error) {
    res.json({ error: "sql执行错误" });
...
router.post("/note", async (req, res) => {
  try {
    ...
    // 执行SQL语句
    const sql = `INSERT INTO \`note\` (id, column_id, content) VALUES ('${uuidv4()}', '${columnId}', '${content}');`;
    const result = await query(`${sql}`);
    res.json({ res: "success" });
  } catch (error) {
    res.json({ error: "sql执行错误" });
...
router.delete("/column", async (req, res) => {
  try {
    ...
    const sql = `delete from \`column\` where id =${columnId}; delete from \`note\` where column_id =${columnId};`;
    const result = await query(`${sql}`);
    res.json({ res: "success" });
  } catch (error) {
    res.json({ error: "sql执行错误" });
